...
namespace cgfFixer
{
    class Fixer_LY_1_12_0_1
    {
        ...
        static void editTempFileHeaderOffsets(int afterElement,uint addSize, uint delta, BinaryReader br)
        {
            ...
            for (int i = 0; i < headerElements; i++)
            {
                ...
                if (id == afterElement)
                {
                    ...
                    streamTempFile.Write(BitConverter.GetBytes(currSize + addSize), 0, sizeof(uint));
                }
                else
                {
                    br.ReadUInt32();
        ...
        private static void fixTangents7(string path)
        {
            ...
            for (int i = 0; i < dataStreamsCount; i++)
            {
                ...
                if (nStreamType == 0x00000006)
                {
                    ...
                    editTempFileHeaderOffsets((int)dataStreamChunksOffsets[i, 1], tangentsChunksOffsets[tangentsCount, 1] * 8, delta, br);
        ...
        private static void fixMesh(string path)
        {
            ...
            //0 - offset
            //1 - id
            uint[,] meshChunks_nStreamChunkIDs = new uint[headerElements, 16];
            ...
            uint delta = 0;
            uint[,] meshChunks_footers = new uint[headerElements, 128];
            for (int i = 0; i < meshChunksCount; i++)
            {
                ...
                for(int j=0;j<16;j++)
                {
                    meshChunks_nStreamChunkIDs[i, j] = br.ReadUInt32();
                }
                for (int j = 0; j < 42; j++)
                {
                    meshChunks_footers[i, j] = br.ReadUInt32();
                }
                long temppos = br.BaseStream.Position;
                delta = delta + (712-260);//16*7*4;//nStreamChunkID elements * LY additional elements * int size
                editTempFileHeaderOffsets((int)meshChunksOffsets[i, 1], (712 - 260), delta, br);
                br.BaseStream.Position = temppos;
            }
            br.Close();
            //BinaryWriter bw = new BinaryWriter(File.Open(
            ...
            {
                uint offset = meshChunksOffsets[i, 0];
                if (i > 0) delta2 = delta2 + (712 - 260);
                offset = offset + delta2;
                streamTempFile.Position = offset;
                streamTempFile.Position = streamTempFile.Position + 28;
                for (int j = 0; j < 16; j++)
                {
                    //Console.WriteLine("{0:X}", streamTempFile.Position);
                    uint zero = 0;
                    streamTempFile.Write(BitConverter.GetBytes(meshChunks_nStreamChunkIDs[i, j]), 0, sizeof(int));
                    streamTempFile.Write(BitConverter.GetBytes(zero), 0, sizeof(int));
                    streamTempFile.Write(BitConverter.GetBytes(zero), 0, sizeof(int));
                    streamTempFile.Write(BitConverter.GetBytes(zero), 0, sizeof(int));
                    streamTempFile.Write(BitConverter.GetBytes(zero), 0, sizeof(int));
                    streamTempFile.Write(BitConverter.GetBytes(zero), 0, sizeof(int));
                ...
                for (int j = 0; j < 43; j++)
                {
                    streamTempFile.Write(BitConverter.GetBytes(meshChunks_footers[i, j]), 0, sizeof(int)); 
                }
                /////////////////////
                //uint offset = meshChunksOffsets[i, 0];
                //bw.BaseStream.Position = offset;
                ...
                //}
                long writePos = streamTempFile.Position;
                appendFooter(path, meshChunksOffsets[i, 0] +  260, writePos);
            }
        ...
        //x0801 for datastreams
        //x0802 for mesh (compiled)
        static void ModifyDataStreamsVersions(string path)
        {
            while (!FixerHelper.IsFileReady(path)) { }
            BinaryReader br = new BinaryReader(File.Open(
                    path, FileMode.Open, FileAccess.Read));
            ...
            uint headerElements = br.ReadUInt32();
            br.ReadUInt32();
            long[] dataStreamHeadersOffsets = new long[headerElements];
            long[] meshCompiledHeadersOffsets = new long[headerElements];
            int dataStreamsCount = 0;
            int meshCompiledCount = 0;
            for (int i = 0; i < (int)headerElements; i++)
            {
                uint chunkType = br.ReadUInt16();
                //Console.WriteLine("{0:X}", chunkType);
                if (chunkType == 0x00001016) { dataStreamHeadersOffsets[dataStreamsCount] = br.BaseStream.Position-2; dataStreamsCount++; }
                if (chunkType == 0x00001000) { meshCompiledHeadersOffsets[meshCompiledCount] = br.BaseStream.Position-2; meshCompiledCount++; }
                br.ReadUInt16();
                br.ReadUInt32();
                br.ReadUInt32();
                br.ReadUInt32();
            ...
            for(int i=0;i<dataStreamsCount;i++)
            {
                //Console.WriteLine("%d {0:X}", dataStreamHeadersOffsets[i],dataStreamHeadersOffsets[i]);
                bw.BaseStream.Position = dataStreamHeadersOffsets[i];
                bw.BaseStream.Position = bw.BaseStream.Position + 2;
                ushort ver = 0x0801;
                bw.Write(ver); 
            }
            for (int i = 0; i < meshCompiledCount; i++)
            {
               // Console.WriteLine("%d {0:X}", meshCompiledHeadersOffsets[i], meshCompiledHeadersOffsets[i]);
                bw.BaseStream.Position = meshCompiledHeadersOffsets[i];
                bw.BaseStream.Position = bw.BaseStream.Position + 2;
                ushort ver= 0x0802;
                bw.Write(ver);
                bw.BaseStream.Position = bw.BaseStream.Position + 4;
                uint size = 0x000002C8;
                bw.Write(size);
            }
            bw.Close();
        }
        static void ModifyDataStreamsHeaders(string path)
        {
            while (!FixerHelper.IsFileReady(path)) { }
            using (FileStream fs = File.Open(path, FileMode.Open, FileAccess.Read))
            {
            ...
            //0 - offset
            //1 - id
            uint[,] dataStreamChunksData = new uint[headerElements, 2];
            //0 - nCount
            //1 - nElementSize
            int dataStreamChunksCount = 0;
            for (int i = 0; i < (int)headerElements; i++)
            {
                uint chunkType = br.ReadUInt16();
                //Console.WriteLine("{0:X}", chunkType);
                ...
                br.ReadUInt32();
                uint offset = br.ReadUInt32();
                if (chunkType == 0x00001016) { dataStreamChunksOffsets[dataStreamChunksCount, 0] = offset; dataStreamChunksOffsets[dataStreamChunksCount, 1] = id; dataStreamChunksCount++; }
            }
            uint delta = 0;
            byte[,] dataStreamChunks_footers = new byte[headerElements, 2560000];
            for (int i = 0; i < dataStreamChunksCount; i++)
            {
                uint offset = dataStreamChunksOffsets[i, 0];
                br.BaseStream.Position = offset;
                br.ReadUInt32();//nFlags
                br.ReadUInt32();//nStreamType
                uint nCount = br.ReadUInt32();//nCount
                uint nElementSize = br.ReadUInt32();//nElementSize
                br.ReadUInt32();//reserved
                br.ReadUInt32();//reserved
                dataStreamChunksData[i, 0] = nCount;
                dataStreamChunksData[i, 1] = nElementSize;
                for (int j = 0; j < nCount* nElementSize; j++)
                {
                    dataStreamChunks_footers[i, j] = br.ReadByte();
                }
                long temppos = br.BaseStream.Position;
                delta = delta + 4;//unknown element size
                editTempFileHeaderOffsets((int)dataStreamChunksOffsets[i, 1],4, delta, br);
                br.BaseStream.Position = temppos;
            }
            br.Close();
            //BinaryWriter bw = new BinaryWriter(File.Open(
            ...
            for (int i = 0; i < dataStreamChunksCount; i++)
            {
                uint zero = 0;
                uint offset = dataStreamChunksOffsets[i, 0];
                if (i > 0) delta2 = delta2 + + 4;
                offset = offset + delta2;
                streamTempFile.Position = offset;
                streamTempFile.Position = streamTempFile.Position + 8;
                uint nCount = dataStreamChunksData[i, 0];//nCount
                uint nElementSize = dataStreamChunksData[i, 1];//nElementSize
                streamTempFile.Write(BitConverter.GetBytes(zero), 0, sizeof(int));//unknown
                streamTempFile.Write(BitConverter.GetBytes(nCount), 0, sizeof(int));//nCount
                streamTempFile.Write(BitConverter.GetBytes(nElementSize), 0, sizeof(int));//nElementSize
                streamTempFile.Write(BitConverter.GetBytes(zero), 0, sizeof(int));//reserved
                streamTempFile.Write(BitConverter.GetBytes(zero), 0, sizeof(int));//reserved
                for (int j = 0; j < nCount* nElementSize; j++)
                {
                    streamTempFile.Write(BitConverter.GetBytes(dataStreamChunks_footers[i, j]), 0, sizeof(byte));
                }
                /////////////////////
                //uint offset = meshChunksOffsets[i, 0];
                //bw.BaseStream.Position = offset;
                ...
                //}
                long writePos = streamTempFile.Position;
                appendFooter(path, dataStreamChunksOffsets[i, 0] + (nCount * nElementSize)+24, writePos);
            }
